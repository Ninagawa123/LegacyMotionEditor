#!/usr/bin/env python3
"""
File Name: LegacyMotionEditor_Importer.py
Description: URDF/MJCF Import functionality for LegacyMotionEditor.
...
# ============================================================================
# UTILITY FUNCTIONS (migrated from urdf_kitchen_utils.py)
# ============================================================================
...
def match_package_name(package_name, dir_name):
    ...
    # Package name contained in dir name
    if package_lower in dir_lower:
        return True

    # Dir name contained in package name
...
class URDFParser:
    ...
    def _extract_xacro_macro_info(self, xacro_file_path):
        """Extract macro name and parameters from a xacro file.

        Args:
            xacro_file_path: Path to xacro file

        Returns:
            dict or None: {'name': macro_name, 'params': params_string} or None if no macro found
        """
        try:
            with open(xacro_file_path, 'r', encoding='utf-8') as f:
                content = f.read()

            # Parse XML to find xacro:macro elements
            # Find xacro:macro element with name attribute
            # Use [\s\S]*? to match multi-line params attribute values
            macro_pattern = r'<xacro:macro\s+name=["\']([^"\']+)["\']\s+params=["\']([\s\S]*?)["\']'
            match = re.search(macro_pattern, content)

            if match:
                macro_name = match.group(1)
                params_str = match.group(2)
                return {'name': macro_name, 'params': params_str}

            return None
        except Exception as e:
            if self.verbose:
                print(f"  Error extracting macro info: {e}")
            return None

    def _create_macro_wrapper_xacro(self, xacro_file_path, macro_info):
        """Create a wrapper xacro file that instantiates the macro.

        Args:
            xacro_file_path: Path to original xacro file
            macro_info: Dict with 'name' and 'params' keys

        Returns:
            str: Path to temporary wrapper xacro file
        """
        macro_name = macro_info['name']
        params_str = macro_info['params']

        # Parse parameters to determine required vs optional
        # Format: "param1='' param2 *param3 param4"
        # - param1='' is optional with default
        # - param2 is required
        # - *param3 is a block parameter
        # - param4 is required

        # Build the macro call
        macro_call_params = []

        # Split params by whitespace, handling multi-line
        ...
        for param in params:
            ...
            if '=' in param:
                # Has default, can be omitted
                param_name = param.split('=')[0]
                macro_call_params.append(f'{param_name}=""')
            elif param.startswith('*'):
                # Block parameter - needs special handling
                block_name = param[1:]  # Remove *
                # For 'origin', provide a default origin block
                if block_name == 'origin':
                    macro_call_params.append(f'<!-- {block_name} block -->')
            else:
                # Required parameter - we need to provide a value
                # Common patterns: 'parent', 'child' need link names
                if param == 'parent':
                    macro_call_params.append('parent="world"')
                elif param == 'child':
                    macro_call_params.append('child="tool0"')
                else:
                    macro_call_params.append(f'{param}=""')

        # Build wrapper xacro content
        xacro_dir = os.path.dirname(os.path.abspath(xacro_file_path))
        xacro_abs_path = os.path.abspath(xacro_file_path)

        # Determine robot name from macro name or filename
        robot_name = macro_name.replace('_', ' ').title().replace(' ', '_')

        wrapper_content = f'''<?xml version="1.0"?>
<robot xmlns:xacro="http://wiki.ros.org/xacro" name="{robot_name}">
  <!-- Include the macro definition -->
  <xacro:include filename="{xacro_abs_path}"/>

  <!-- Create world link as parent -->
  <link name="world"/>

  <!-- Create tool link as child -->
  <link name="tool0"/>

  <!-- Instantiate the robot macro -->
  <xacro:{macro_name} {' '.join(p for p in macro_call_params if not p.startswith('<!--'))}>
    <origin xyz="0 0 0" rpy="0 0 0"/>
  </xacro:{macro_name}>
</robot>
'''

        # Write to temporary file in same directory (for relative includes to work)
        ...
        if self.verbose:
            print(f"  Created wrapper xacro: {temp_file.name}")
    ...
    def _expand_xacro(self, xacro_file_path):
        ...
        try:
            ...
            xacro_file_abs = os.path.abspath(xacro_file_path)
            wrapper_file = None  # Track wrapper file for cleanup
            ...
            if urdf_xml_string:
                try:
                    ...
                    # Check if this is an empty robot (macro-only file)
                    if link_count == 0 and joint_count == 0:
                        if self.verbose:
                            print(f"  No links or joints found - checking if this is a macro-only file...")

                        # Try to extract macro info and create wrapper
                        macro_info = self._extract_xacro_macro_info(xacro_file_abs)
                        if macro_info:
                            if self.verbose:
                                print(f"  Found macro: {macro_info['name']} with params: {macro_info['params']}")

                            # Create wrapper xacro that instantiates the macro
                            wrapper_file = self._create_macro_wrapper_xacro(xacro_file_abs, macro_info)

                            try:
                                # Expand the wrapper instead
                                wrapper_doc = XacroDoc.from_file(wrapper_file)
                                urdf_xml_string = wrapper_doc.to_urdf_string()
                                ...
                                if self.verbose:
                                    print(f"  After wrapper expansion: {link_count} links, {joint_count} joints")
                            finally:
...
class MJCFParser:
    ...
    def parse_mjcf(self, mjcf_file_path, working_dir=None):
        ...
        self.mjcf_materials = self._parse_materials_ir(root, mjcf_file_path)
    ...
    def _parse_defaults(self, root):
        ...
        def parse_defaults_recursive(default_elem, parent_class_name=None, ancestor_classes=None):
            ...
            if geom_elem is not None:
                ...
                material_str = geom_elem.get('material')
                if material_str:
                    geom_defaults['material'] = material_str

                rgba_str = geom_elem.get('rgba')
                if rgba_str:
                    geom_defaults['rgba'] = rgba_str
    ...
    def _parse_with_includes(self, mjcf_file_path, working_dir=None):
        ...
        try:
            # Parse main file — fall back to latin-1 decode when the file
            # contains non-UTF-8 bytes (e.g. Shift-JIS comments).
            try:
                tree = ET.parse(mjcf_file_path)
                root = tree.getroot()
            except ET.ParseError:
                import re as _re
                with open(mjcf_file_path, 'rb') as _f:
                    _raw = _f.read()
                _text = _raw.decode('latin-1', errors='replace')
                _text = _re.sub(r'<\?xml[^?]*\?>', '', _text)
    ...
    def _parse_body(self, body_elem, parent_name, level, bodies_data, joints_data,
        ...
        for idx, geom_elem in enumerate(geom_elems):
            ...
            if mesh_name and mesh_name in meshes_data:
                ...
                # Parse color
                geom_color = [1.0, 1.0, 1.0, 1.0]
                geom_material = None

                if geom_class and geom_class in default_classes:
                    geom_defaults = default_classes[geom_class].get('geom', {})
                    geom_material = geom_defaults.get('material')
                    if 'rgba' in geom_defaults:
                        rgba = [float(v) for v in geom_defaults['rgba'].split()]
                        geom_color = rgba[:4] if len(rgba) >= 4 else rgba[:3] + [1.0]
                elif None in default_classes:
                    geom_defaults = default_classes[None].get('geom', {})
                    geom_material = geom_defaults.get('material')
                    if 'rgba' in geom_defaults:
                        rgba = [float(v) for v in geom_defaults['rgba'].split()]
                        geom_color = rgba[:4] if len(rgba) >= 4 else rgba[:3] + [1.0]

                direct_material = geom_elem.get('material')
                if direct_material:
                    geom_material = direct_material

                material_data = getattr(self, 'mjcf_materials', {}).get(geom_material) if geom_material else None
                if material_data and 'rgba' in material_data:
                    rgba = material_data['rgba']
                    geom_color = rgba[:4] if len(rgba) >= 4 else rgba[:3] + [1.0]
        ...
        for joint_elem in joint_elems:
            ...
            joint_data = {
                ...
                'axis': axis_local,  # Axis in body local frame (for FK rotation)
...
def select_and_parse_urdf(parent_widget=None):
    """Select a URDF/xacro/SDF/SRDF file and return (urdf_file, working_dir, urdf_data).

    This function handles:
    - URDF files (.urdf)
    - Xacro files (.xacro) - expanded to URDF using xacrodoc
    - SDF files (.sdf) - parsed using SDFParser
    - SRDF files (.srdf) - searches for corresponding URDF/SDF file

    Returns:
        tuple: (urdf_file_path, working_dir, urdf_data) or None if cancelled
    """
    urdf_file, _ = QtWidgets.QFileDialog.getOpenFileName(
        parent_widget,
        "Select URDF, xacro, SDF, or SRDF file to import",
    ...
    return parse_urdf_file(urdf_file, parent_widget=parent_widget)


def parse_urdf_file(urdf_file, parent_widget=None):
    """Parse a URDF/xacro/SDF file without creating any NodeGraph nodes.

    Args:
        urdf_file: Path to URDF/xacro/SDF/SRDF file
        parent_widget: Parent widget for dialogs

    Returns:
        tuple: (urdf_file_path, working_dir, urdf_data) or None if failed
    """
    working_dir = os.path.dirname(os.path.abspath(urdf_file))

    # Check file type
    file_ext = os.path.splitext(urdf_file)[1].lower()
    is_xacro = file_ext in ['.xacro', '.xacro.urdf'] or '.xacro' in os.path.basename(urdf_file).lower()
    is_srdf = file_ext == '.srdf'
    is_sdf = file_ext == '.sdf'

    print(f"Parsing URDF/xacro/SDF from: {urdf_file}")
    print(f"  File extension: {file_ext}, is_xacro={is_xacro}, is_sdf={is_sdf}, is_srdf={is_srdf}")

    if is_srdf:
        print(f"  Detected SRDF file, will search for corresponding URDF/SDF file")
        # For SRDF file, find corresponding URDF/SDF file
        srdf_dir = os.path.dirname(urdf_file)
        srdf_basename = os.path.splitext(os.path.basename(urdf_file))[0]

        # Search for URDF/SDF files
        def search_urdf_sdf_files(base_dir, basename, max_depth=4):
            """Search for URDF/SDF files up to max_depth levels"""
            candidates = []

            # Search in same directory
            candidates.extend([
                os.path.join(base_dir, f"{basename}.urdf"),
                os.path.join(base_dir, f"{basename}.xacro"),
                os.path.join(base_dir, f"{basename}.sdf"),
            ])
            ...
            # Search up to max_depth levels down
            base_depth = len(base_dir.split(os.sep))
            for root_dir, dirs, files in os.walk(base_dir):
                current_depth = len(root_dir.split(os.sep)) - base_depth
                if current_depth > max_depth:
    ...
    try:
        if is_sdf:
            print(f"  Parsing SDF file using SDFParser...")
            sdf_parser = SDFParser(verbose=True)
            sdf_data = sdf_parser.parse_sdf(urdf_file)
            # Convert SDF data to URDF format
            urdf_data = {
                'robot_name': sdf_data['robot_name'],
                'links': sdf_data['links_data'],
                'joints': sdf_data['joints_data'],
                'closed_loop_joints': sdf_data.get('closed_loop_joints', []),
                'materials': sdf_data['materials_data'],
                'missing_meshes': sdf_data['missing_meshes']
            }
        else:
            if is_xacro:
                print(f"  Expanding xacro file...")
            else:
                print(f"  Parsing URDF file...")
            urdf_parser = URDFParser(verbose=True)
            urdf_data = urdf_parser.parse_urdf(urdf_file)
    except FileNotFoundError as e:
        error_msg = f"Could not find URDF/xacro file:\n{urdf_file}\n\n{str(e)}"
        ...
        if parent_widget:
            if "xacro" in error_msg.lower():
                QtWidgets.QMessageBox.critical(
                    ...
                    "Xacro Expansion Failed",
                    f"Failed to expand xacro file:\n{urdf_file}\n\n{error_msg}\n\n"
                    "Please ensure xacrodoc is installed:\n"
                    "  pip install xacrodoc"
                )
            else:
                QtWidgets.QMessageBox.critical(
        ...
        error_msg = f"Failed to parse XML in file:\n{urdf_file}\n\n{str(e)}"
        print(f"Error: {error_msg}")
        if parent_widget:
            QtWidgets.QMessageBox.critical(
                parent_widget,
        ...
        error_msg = f"Unexpected error while importing URDF:\n{urdf_file}\n\n{str(e)}"
        print(f"Error: {error_msg}")
        traceback.print_exc()
        if parent_widget:
            QtWidgets.QMessageBox.critical(
    ...
    if srdf_file:
        try:
            srdf_parser = SRDFParser(verbose=True)
            srdf_data = srdf_parser.parse_srdf(srdf_file)
            urdf_data['srdf'] = srdf_data
            print(f"\n=== SRDF Parse Summary ===")
            print(f"Groups: {len(srdf_data.get('groups', []))}")
            print(f"Disabled collisions: {len(srdf_data.get('disabled_collisions', []))}")
            print(f"End effectors: {len(srdf_data.get('end_effectors', []))}")
            print("=" * 30 + "\n")
        except Exception as e:
            print(f"Warning: Failed to parse SRDF file: {srdf_file}\n{str(e)}")
            traceback.print_exc()
    ...
    print(f"\n=== URDF Parse Summary ===")
    print(f"Robot name: {urdf_data.get('robot_name', 'unknown')}")
    print(f"Links: {len(urdf_data.get('links', []))}")
    print(f"Joints: {len(urdf_data.get('joints', []))}")
    print(f"Missing meshes: {len(urdf_data.get('missing_meshes', []))}")
    print("=" * 30 + "\n")

    return urdf_file, working_dir, urdf_data


def select_and_parse_mjcf(parent_widget=None):
        """Select an MJCF XML/ZIP file and return (mjcf_file, working_dir, mjcf_data)."""
        ...
        return parse_mjcf_file(mjcf_file, parent_widget=parent_widget)


def parse_mjcf_file(mjcf_file, parent_widget=None):
        """Parse an MJCF XML/ZIP file without creating any NodeGraph nodes."""
        working_dir = None
        selected_file = mjcf_file
        ...
        if mjcf_file.endswith('.zip'):
            ...
            try:
                ...
                if len(xml_files) > 1:
                    ...
                    selected_rel, ok = QtWidgets.QInputDialog.getItem(
                        parent_widget,
                        "Select XML File",
                    ...
                    if not ok or not selected_rel:
                        return None
                    selected_file = os.path.join(temp_dir, selected_rel)
                else:
        ...
        mjcf_data = mjcf_parser.parse_mjcf(selected_file, working_dir=working_dir)
        return selected_file, working_dir, mjcf_data


def detect_model_type(file_path):
    """Detect model file type from extension and content.

    Args:
        file_path: Path to the model file

    Returns:
        str: 'urdf', 'mjcf', 'sdf', 'xacro', or None if unknown
    """
    ...
    if ext == '.urdf':
        return 'urdf'
    elif ext == '.xacro' or '.xacro' in basename:
        return 'xacro'
    elif ext == '.sdf':
        return 'sdf'
    elif ext == '.srdf':
        ...
        return 'mjcf'  # Assume ZIP contains MJCF
    elif ext == '.xml':
        # Read raw bytes decoded as latin-1 (never fails) to find root tag
        # without full XML parsing — tolerates Shift-JIS or other non-UTF-8 bytes
        # in comments or strings that would otherwise cause ET.parse to throw.
        try:
            import re as _re
            with open(file_path, 'rb') as _f:
                _head = _f.read(8192).decode('latin-1', errors='replace')
            # First element tag, skipping <?xml?> declarations and <!-- comments -->
            _m = _re.search(r'<(?!\?|![-\[])\s*([A-Za-z_][\w.-]*)', _head)
            root_tag = _m.group(1).lower() if _m else ''
            if root_tag == 'mujoco':
                return 'mjcf'
            elif root_tag in ('sdf', 'gazebo'):
                return 'sdf'
            elif root_tag == 'robot':
                return 'urdf'
            elif root_tag:
                return 'mjcf'  # unknown XML — best-effort fallback
        except Exception:
            pass
        return None
...
def select_and_parse_model(parent_widget=None):
    """Select and parse any supported robot model file.

    Supports: URDF, Xacro, SDF, SRDF, MJCF (XML/ZIP)

    Returns:
        tuple: (file_path, working_dir, parsed_data, model_type) or None if cancelled
               model_type is one of: 'urdf', 'xacro', 'sdf', 'mjcf'
    """
    file_path, _ = QtWidgets.QFileDialog.getOpenFileName(
        parent_widget,
        "Select Robot Model File",
        os.getcwd(),
        "All Model Files (*.urdf *.xacro *.sdf *.srdf *.xml *.zip);;"
        "URDF Files (*.urdf);;"
        "Xacro Files (*.xacro);;"
        "SDF Files (*.sdf);;"
        "MJCF Files (*.xml *.zip);;"
    ...
    return parse_model_file(file_path, parent_widget=parent_widget)


def parse_model_file(file_path, parent_widget=None):
    """Parse a robot model file of any supported type.
    ...
    Returns:
        tuple: (file_path, working_dir, parsed_data, model_type) or None if failed
    """
    ...
    if model_type is None:
        QtWidgets.QMessageBox.warning(
            parent_widget,
            "Unknown File Type",
            f"Could not determine the file type of:\n{file_path}\n\n"
            "Supported formats: URDF, Xacro, SDF, SRDF, MJCF (XML/ZIP)"
        )
        return None

    print(f"[ModelImport] Detected model type: {model_type} for {file_path}")

    if model_type in ['urdf', 'xacro', 'sdf', 'srdf']:
        # Use URDF parser (handles all URDF-family formats)
        result = parse_urdf_file(file_path, parent_widget=parent_widget)
        if result:
            urdf_path, working_dir, urdf_data = result
            # Keep original model_type (urdf, xacro, sdf, srdf) for Save/Load
            return urdf_path, working_dir, urdf_data, model_type
        return None

    elif model_type == 'mjcf':
        # Use MJCF parser
        result = parse_mjcf_file(file_path, parent_widget=parent_widget)
        if result:
            mjcf_path, working_dir, mjcf_data = result
            return mjcf_path, working_dir, mjcf_data, 'mjcf'
        return None
...
def convert_model_names(
    ...
    *,
    ...
    """Assign canonical short joint/link names to a parsed URDF/MJCF model.

    Thin wrapper around robot_name_converter.importer_bridge.convert_parsed_model.
    """
    from robot_name_converter import convert_parsed_model

    return convert_parsed_model(
        parsed_data,
        model_type,
...
def parse_and_convert_model_names(file_path, parent_widget=None, **converter_kwargs):
    """Parse any supported model file and return canonical name assignments."""
    from robot_name_converter import convert_model_file

    return convert_model_file(file_path, parent_widget=parent_widget, **converter_kwargs)
...
# ============================================================================
# LME Robot Model (VTK actors + FK; moved from LegacyMotionEditor_Utils.py)
# ============================================================================
...
class URDFJoint:
    ...
    def __init__(self):
        ...
        self.limit_lower = -180.0
        self.limit_upper = 180.0


class URDFLink:
    """URDFのlink情報を保持（複数のvisualをサポート）"""
    def __init__(self):
        self.name = ""
        # 後方互換性のために保持
        ...
        # 複数のvisual要素を保持
        self.visuals = []  # List of {'mesh': path, 'origin_xyz': [], 'origin_rpy': [], 'color': []}


class URDFRobotModel:
    """URDFをパースし、VTKアクター管理とFK計算を行うクラス"""
    ...
    def _find_mesh_file(self, mesh_basename, search_dirs=None):
        ...
        1. URDFと同じディレクトリ
        2. 親ディレクトリ
        3. 親ディレクトリの子ディレクトリ（meshes等）
        4. 親ディレクトリの孫ディレクトリ
        5. 祖父母ディレクトリとその子・孫ディレクトリ

        Args:
            mesh_basename: メッシュファイル名（例: c_waist.stl）
            search_dirs: 追加の検索ディレクトリリスト
        ...
        # 1. URDFと同じディレクトリ
        candidates.append(self.urdf_dir)
        ...
        if parent_dir:
            ...
            # 3. 親ディレクトリの子ディレクトリ（meshes, mesh, stl等）
            try:
                for item in os.listdir(parent_dir):
                        ...
                        try:
                            for subitem in os.listdir(item_path):
                                ...
                                if os.path.isdir(subitem_path):
                                    candidates.append(subitem_path)
        ...
        if grandparent_dir and grandparent_dir != parent_dir:
            candidates.append(grandparent_dir)
            try:
        ...
        # 各候補ディレクトリでファイルを検索
        for search_dir in unique_candidates:
            candidate_path = os.path.join(search_dir, mesh_basename)
            if os.path.exists(candidate_path):
                return os.path.normpath(candidate_path)

        # 6. 親ディレクトリから6階層まで深い検索（os.walk使用）
    ...
    def parse_urdf(self, urdf_path):
        """URDFファイルをパースする（マテリアル・複数visual対応）"""
        ...
        for mat_elem in root.findall('material'):
            ...
            if color_elem is not None:
                ...
                rgba = [float(v) for v in rgba_str.split()]
                self.materials[mat_name] = rgba[:3]

        # リンクのパース（全てのvisual要素を処理）
        for link_elem in root.findall('link'):
            lk = URDFLink()
            lk.name = link_elem.get('name', '')
            ...
            for vis in link_elem.findall('visual'):
                ...
                if origin is not None:
                    vis_data['origin_xyz'] = [float(v) for v in origin.get('xyz', '0 0 0').split()]
                    vis_data['origin_rpy'] = [float(v) for v in origin.get('rpy', '0 0 0').split()]
                ...
                if mat_elem is not None:
                    ...
                    if color_elem is not None:
                        rgba_str = color_elem.get('rgba', '0.8 0.8 0.8 1.0')
                        rgba = [float(v) for v in rgba_str.split()]
                        vis_data['color'] = rgba[:3]
                    elif mat_name in self.materials:
                        vis_data['color'] = self.materials[mat_name]

                if vis_data['mesh']:
                    lk.visuals.append(vis_data)
                    # 後方互換性：最初のvisualを従来のフィールドにも設定
                    if lk.visual_mesh is None:
                        lk.visual_mesh = vis_data['mesh']
                        lk.visual_origin_xyz = vis_data['origin_xyz']
                        lk.visual_origin_rpy = vis_data['origin_rpy']
        ...
        for joint_elem in root.findall('joint'):
            jt = URDFJoint()
            jt.name = joint_elem.get('name', '')
            jt.joint_type = joint_elem.get('type', 'fixed')
            parent_elem = joint_elem.find('parent')
            child_elem = joint_elem.find('child')
            if parent_elem is not None:
                jt.parent_link = parent_elem.get('link', '')
            if child_elem is not None:
                jt.child_link = child_elem.get('link', '')
            origin = joint_elem.find('origin')
            if origin is not None:
                xyz_str = origin.get('xyz', '0 0 0')
                rpy_str = origin.get('rpy', '0 0 0')
                jt.origin_xyz = [float(v) for v in xyz_str.split()]
                jt.origin_rpy = [float(v) for v in rpy_str.split()]
            axis_elem = joint_elem.find('axis')
            if axis_elem is not None:
                jt.axis = [float(v) for v in axis_elem.get('xyz', '1 0 0').split()]
            limit_elem = joint_elem.find('limit')
            if limit_elem is not None:
                jt.limit_lower = math.degrees(float(limit_elem.get('lower', str(math.radians(-180)))))
                jt.limit_upper = math.degrees(float(limit_elem.get('upper', str(math.radians(180)))))
            else:
                jt.limit_lower = -180.0
                jt.limit_upper = 180.0
            self.joints[jt.name] = jt
            self.parent_map[jt.child_link] = jt.name
            if jt.parent_link not in self.child_map:
                self.child_map[jt.parent_link] = []
            self.child_map[jt.parent_link].append(jt.name)
            child_links.add(jt.child_link)
            if jt.joint_type == 'revolute':
                self.joint_order.append(jt.name)

        # ルートリンクの特定
        all_link_names = set(self.links.keys())
        roots = all_link_names - child_links
        if roots:
            self.root_link = list(roots)[0]
        elif self.links:
            self.root_link = list(self.links.keys())[0]
        print(f"[URDFRobotModel] Parsed {len(self.links)} links, "
              f"{len(self.joints)} joints, "
              f"{len(self.joint_order)} revolute joints, "
              f"root={self.root_link}")

    def _create_mesh_reader(self, mesh_path):
        """Create a VTK reader for supported mesh formats.

        Supports: STL, OBJ, PLY (via VTK readers), DAE (via trimesh conversion)

        Returns:
            VTK reader object, or None if format is unsupported
        """
        ...
        if ext == ".stl":
            ...
            # DAE (COLLADA) files: convert to STL using trimesh
            return self._load_dae_mesh(mesh_path)
        else:
            print(f"[URDFRobotModel] Unsupported mesh format: {mesh_path}")
            return None
        reader.SetFileName(mesh_path)
        return reader

    def _load_dae_mesh(self, mesh_path):
        """Load DAE (COLLADA) mesh using trimesh and convert to VTK.

        Args:
            mesh_path: Path to DAE file
        ...
        try:
            ...
            # Load the temporary STL with VTK
            reader = vtk.vtkSTLReader()
            reader.SetFileName(temp_stl_path)

            # Store temp file path for cleanup (optional)
            if not hasattr(self, '_temp_mesh_files'):
                self._temp_mesh_files = []
            self._temp_mesh_files.append(temp_stl_path)
    ...
    def _resolve_mesh_path(self, filename):
        ...
        検索順序:
        1. package://パスを解析してパッケージルートを検索
        2. 相対パスをURDFディレクトリから解決
        3. 親ディレクトリから複数候補を試す
        4. ファイルベース名で全階層検索

        Args:
            filename: メッシュファイルパス（package://形式または相対パス）

        Returns:
            解決されたファイルパス、見つからない場合は元のパス
        """
        if not filename:
            return None
        ...
        if filename.startswith('package://'):
            pkg_path = filename[len('package://'):]

            if pkg_path.startswith('../') or pkg_path.startswith('..\\'):
                # 相対パス形式
                candidates.append(os.path.normpath(os.path.join(self.urdf_dir, pkg_path)))
            else:
                # package://package_name/path/to/mesh 形式
                parts = pkg_path.split('/', 1)
                package_name = parts[0] if parts else ''
                rel_path = parts[1] if len(parts) > 1 else pkg_path

                # 候補1: 親ディレクトリ直下
                parent_dir = os.path.dirname(self.urdf_dir)
                candidates.append(os.path.normpath(os.path.join(parent_dir, rel_path)))

                # 候補2: URDFディレクトリ直下
                candidates.append(os.path.normpath(os.path.join(self.urdf_dir, rel_path)))

                # 候補3: 祖父母ディレクトリからパッケージ名検索
                grandparent_dir = os.path.dirname(parent_dir) if parent_dir else None
                if grandparent_dir and os.path.isdir(grandparent_dir):
                    try:
                        for item in os.listdir(grandparent_dir):
                            item_path = os.path.join(grandparent_dir, item)
                            if os.path.isdir(item_path):
                                # パッケージ名との一致をチェック（部分一致も含む）
                                item_lower = item.lower().replace('_', '').replace('-', '')
                                pkg_lower = package_name.lower().replace('_', '').replace('-', '')
                                if pkg_lower in item_lower or item_lower in pkg_lower:
                                    candidates.append(os.path.normpath(os.path.join(item_path, rel_path)))
                    except (PermissionError, OSError):
                        pass

                # 候補4: robots/プレフィックスを除去して検索
                if rel_path.startswith('robots/'):
                    rel_path_without_robots = rel_path[len('robots/'):]
                    candidates.append(os.path.normpath(os.path.join(parent_dir, rel_path_without_robots)))
                    if grandparent_dir:
                        candidates.append(os.path.normpath(os.path.join(grandparent_dir, rel_path_without_robots)))

        elif filename.startswith('file://'):
            # file://形式
            file_path = filename[len('file://'):]
            candidates.append(os.path.normpath(file_path))
            candidates.append(os.path.normpath(os.path.join(self.urdf_dir, file_path)))
        else:
            # 相対パスまたは絶対パス
            if os.path.isabs(filename):
                candidates.append(filename)
            else:
                candidates.append(os.path.normpath(os.path.join(self.urdf_dir, filename)))
                # 親ディレクトリからも試す
                parent_dir = os.path.dirname(self.urdf_dir)
                candidates.append(os.path.normpath(os.path.join(parent_dir, filename)))
        ...
        # 見つからない場合、ファイルベース名で検索
        found_path = self._find_mesh_file(mesh_basename)
        if found_path:
            return found_path

        # それでも見つからない場合、最初の候補を返す
        return candidates[0] if candidates else filename

    def build_vtk_actors(self, renderer):
        """各リンクのVTKアクターを生成し、レンダラーに追加（複数visual・色対応）"""
        print(f"[URDFRobotModel] build_vtk_actors called with {len(self.links)} links")
        ...
        def compute_fk(link_name, parent_transform):
            if link_name in visited:
                return  # Already visited, skip to prevent infinite recursion
            visited.add(link_name)
            link_world_transforms[link_name] = parent_transform.copy()
            if link_name in self.child_map:
                for joint_name in self.child_map[link_name]:
                    jt = self.joints[joint_name]
                    # Build transform using VTK-style order (matches Assembler):
                    # Parent @ Translate @ RotateZ @ RotateY @ RotateX
                    # This is equivalent to: rotate the point, then translate, then apply parent
                    # But with VTK's pre-multiply convention

                    # Create joint transform: Translation followed by Rotation
                    # For URDF: child frame is at xyz, oriented by rpy
                    # Matrix: | R  xyz |  where R = Rz @ Ry @ Rx
                    #         | 0   1  |
                    joint_T = make_transform_matrix(jt.origin_xyz, jt.origin_rpy)
                    child_world = parent_transform @ joint_T
                    compute_fk(jt.child_link, child_world)

        if self.root_link:
            compute_fk(self.root_link, np.eye(4))

        # Initialize visual transforms storage
        if not hasattr(self, 'link_visual_transforms'):
            self.link_visual_transforms = {}

        loaded_count = 0
        for link_name, lk in self.links.items():
            world_T = link_world_transforms.get(link_name, np.eye(4))
            link_actors = []
            link_transforms = []
            link_visual_transforms = []  # Store visual transforms for this link

            # 全てのvisual要素を処理
            for vis_data in lk.visuals:
                mesh_path = vis_data['mesh']
                if not mesh_path:
                    continue

                # メッシュパスが存在しない場合、解決を試みる
                if not os.path.exists(mesh_path):
                    # まず_resolve_mesh_pathで試す
                    ...
                    if resolved and os.path.exists(resolved):
                        ...
                        found = self._find_mesh_file(mesh_basename)
                        if found:
                            mesh_path = found
                        else:
                            print(f"[URDFRobotModel] Mesh not found for {link_name}: {vis_data['mesh']}")
                            continue

                try:
                    reader = self._create_mesh_reader(mesh_path)
                    if reader is None:
                        continue
                    reader.Update()
                    ...
                    # Apply visual origin transform to mesh geometry (like Assembler)
                    # This bakes visual origin directly into the mesh vertices
                    vis_xyz = vis_data['origin_xyz']
                    vis_rpy = vis_data['origin_rpy']
                    mesh_scale = vis_data.get('scale', [1.0, 1.0, 1.0])

                    # Build visual origin transform matrix
                    # Transform order: p' = T * R * S * p (Scale, then Rotate, then Translate)
                    roll, pitch, yaw = vis_rpy
                    cr, sr = math.cos(roll), math.sin(roll)
                    cp, sp = math.cos(pitch), math.sin(pitch)
                    cy, sy = math.cos(yaw), math.sin(yaw)

                    # Standard RPY rotation matrix: R = Rz * Ry * Rx
                    R = np.array([
                    ...
                    # Apply scale to rotation (R * S = scale each column by scale factor)
                    sx, sy_scale, sz = mesh_scale
                    RS = R.copy()
                    RS[:, 0] *= sx
                    RS[:, 1] *= sy_scale
                    RS[:, 2] *= sz

                    # Build 4x4 visual transform matrix
                    vis_T = np.eye(4)
                    vis_T[:3, :3] = RS
                    vis_T[0, 3] = vis_xyz[0]
                    vis_T[1, 3] = vis_xyz[1]
                    ...
                    # Combine world transform with visual transform
                    # combined_T = world_T @ vis_T
                    combined_T = world_T @ vis_T
                    ...
                    # Apply combined transform (world @ visual)
                    vtk_matrix = vtk.vtkMatrix4x4()
                    for i in range(4):
                        for j in range(4):
                            vtk_matrix.SetElement(i, j, combined_T[i, j])
                    ...
                    link_actors.append(actor)
                    link_transforms.append(vtk_transform)
                    link_visual_transforms.append(vis_T)  # Store visual transform
                    loaded_count += 1

                except Exception as e:
                    print(f"[URDFRobotModel] Error loading STL for {link_name}: {e}")

            if link_actors:
                self.link_actors[link_name] = link_actors
                self.link_transforms[link_name] = link_transforms
                self.link_visual_transforms[link_name] = link_visual_transforms  # Store visual transforms
                print(f"[URDFRobotModel] Loaded {len(link_actors)} visual(s) for link: {link_name}")
    ...
    def apply_joint_angles(self, angles_deg, snap: bool = False):
        ...
        def compute_fk_with_angles(link_name, parent_transform):
            if link_name in visited:
                return  # Already visited, skip to prevent infinite recursion
            visited.add(link_name)
            link_world_transforms[link_name] = parent_transform.copy()
            if link_name in self.child_map:
                for joint_name in self.child_map[link_name]:
                    jt = self.joints[joint_name]
                    # ジョイントのオリジン変換
                    jt_R = _rpy_to_rotation_matrix(*jt.origin_rpy)
                    jt_T = _make_4x4(jt_R, jt.origin_xyz)
                    # ジョイント回転
                    if jt.joint_type == 'revolute':
                        angle_deg = angles_deg.get(jt.name, 0.0)
                        angle_rad = math.radians(angle_deg)
                        rot_R = _axis_angle_to_rotation_matrix(jt.axis, angle_rad)
                        rot_T = _make_4x4(rot_R, [0, 0, 0])
                    else:
                        rot_T = np.eye(4)
                    child_world = parent_transform @ jt_T @ rot_T
                    compute_fk_with_angles(jt.child_link, child_world)

        compute_fk_with_angles(self.root_link, np.eye(4))

        # Cache for external use (e.g. 3D view drag direction computation)
        self._link_world_transforms = link_world_transforms

        # 各リンクのアクターの変換を更新
        for link_name, lk in self.links.items():
            if link_name not in self.link_actors or link_name not in self.link_transforms:
                continue

            world_T = link_world_transforms.get(link_name, np.eye(4))
            actors = self.link_actors[link_name]
            transforms = self.link_transforms[link_name]

            # Get visual transforms if available
            visual_transforms = getattr(self, 'link_visual_transforms', {}).get(link_name, None)

            for i, (actor, vtk_transform) in enumerate(zip(actors, transforms)):
                # Combine world transform with visual transform
                if visual_transforms and i < len(visual_transforms):
                    vis_T = visual_transforms[i]
                    combined_T = world_T @ vis_T
                else:
                    combined_T = world_T
                ...
                for r in range(4):
                    for c in range(4):
                        vtk_matrix.SetElement(r, c, combined_T[r, c])
                vtk_transform.SetMatrix(vtk_matrix)

    def get_joint_angle(self, joint_name):
        """指定した関節の現在の角度を取得"""
        return self.current_angles.get(joint_name, 0.0)
    ...
    def _fk_recursive(self, link_name, parent_world, angles_deg):
        ...
        # このリンクのビジュアルオフセットを適用
        vis_R = _rpy_to_rotation_matrix(*lk.visual_origin_rpy)
        vis_T = _make_4x4(vis_R, lk.visual_origin_xyz)
        link_world = parent_world @ vis_T

        # VTKトランスフォームを更新（新形式に対応）
        if link_name in self.link_transforms:
            ...
            if isinstance(transforms, list):
                for tf in transforms:
                    ...
                    for i in range(4):
                        for j in range(4):
                            m.SetElement(i, j, link_world[i, j])
                    tf.SetMatrix(m)
            else:
                m = vtk.vtkMatrix4x4()
                for i in range(4):
                    for j in range(4):
                        m.SetElement(i, j, link_world[i, j])
                transforms.SetMatrix(m)

        # 子ジョイントを処理
        joint_names = self.child_map.get(link_name, [])
        for jname in joint_names:
            jt = self.joints[jname]
            # ジョイントのオリジン変換
            jt_R = _rpy_to_rotation_matrix(*jt.origin_rpy)
            jt_T = _make_4x4(jt_R, jt.origin_xyz)
            # ジョイント回転
            if jt.joint_type == 'revolute':
                ...
                rot_T = np.eye(4)
            child_world = parent_world @ jt_T @ rot_T
            self._fk_recursive(jt.child_link, child_world, angles_deg)

    def get_default_angles(self):
        """全関節0度のanglesを返す"""
        return {jname: 0.0 for jname in self.joint_order}


def _is_leg_branch_link(link_name: str) -> bool:
    """Heuristic: link subtree is leg/feet (not arms/torso/head)."""
    ln = link_name.lower().replace("-", "_")
    if any(x in ln for x in (
        "torso", "chest", "spine", "neck", "head", "shoulder", "arm_", "elbow", "hand", "finger", "wing",
    )):
        return False
    if any(x in ln for x in (
        "hipjoint", "hip_joint", "thigh", "shank", "leg_", "knee", "ankle", "foot_", "calf",
    )):
        return True
    if ln.startswith(("l_", "r_", "fl_", "fr_", "bl_", "br_")):
        if any(x in ln for x in ("arm", "shoulder", "elbow", "hand")):
            return False
        if any(x in ln for x in ("hip", "leg", "ankle", "foot", "calf", "thigh")):
            return True
    return False


def _joint_name_is_lower_body(joint_name: str) -> bool:
    jn = joint_name.lower()
    return any(x in jn for x in ("hipjoint", "hip_", "knee", "ankle"))


def get_base_link_anchor_child_links(robot_model) -> list[str]:
    """Return link names directly attached beneath root/base_link."""
    root = getattr(robot_model, "root_link", None) or "base_link"
    if root not in robot_model.links and "base_link" in robot_model.links:
        root = "base_link"
    child_links = []
    for jname in robot_model.child_map.get(root, []):
        jt = robot_model.joints.get(jname)
        if jt and jt.child_link:
            child_links.append(jt.child_link)
    return child_links


def _revolute_joints_in_link_subtree(robot_model, link_name: str) -> list[str]:
    joints: list[str] = []
    for jname in robot_model.child_map.get(link_name, []):
        jt = robot_model.joints.get(jname)
        if not jt:
            continue
        if jt.joint_type == "revolute" and jname in robot_model.joint_order:
            joints.append(jname)
        joints.extend(_revolute_joints_in_link_subtree(robot_model, jt.child_link))
    return joints


def classify_upper_lower_body_joints(robot_model) -> tuple[set[str], set[str]]:
    """Split revolute joints using the first link(s) under base_link as waist anchor.

    Upper: non-leg branches from anchor (torso, head, arms, …).
    Lower: leg branches from anchor (hips through feet).
    """
    upper: set[str] = set()
    lower: set[str] = set()
    if not robot_model:
    ...
    anchors = get_base_link_anchor_child_links(robot_model)
    if not anchors:
        for jname in robot_model.joint_order:
            (lower if _joint_name_is_lower_body(jname) else upper).add(jname)
        return upper, lower

    for anchor in anchors:
        for jname in robot_model.child_map.get(anchor, []):
            jt = robot_model.joints.get(jname)
            if not jt:
                continue
            bundle: list[str] = []
            if jt.joint_type == "revolute" and jname in robot_model.joint_order:
                bundle.append(jname)
            bundle.extend(_revolute_joints_in_link_subtree(robot_model, jt.child_link))
            if _is_leg_branch_link(jt.child_link):
                lower.update(bundle)
            else:
                upper.update(bundle)

    unassigned = set(robot_model.joint_order) - upper - lower
    for jname in unassigned:
        (lower if _joint_name_is_lower_body(jname) else upper).add(jname)
    return upper, lower


def build_robot_model_from_mjcf(mjcf_path, mjcf_data):
    """Build the motion editor's robot model from MJCFParser output."""
    rm = URDFRobotModel()
    rm.urdf_path = mjcf_path
    rm.urdf_dir = os.path.dirname(os.path.abspath(mjcf_path))
    rm.mjcf_data = mjcf_data
    rm.links.clear()
    rm.joints.clear()
    rm.joint_order.clear()
    rm.parent_map.clear()
    rm.child_map.clear()

    def ensure_link(link_name):
        if not link_name:
            return None
        if link_name not in rm.links:
            lk = URDFLink()
            lk.name = link_name
            rm.links[link_name] = lk
        return rm.links[link_name]

    ensure_link("base_link")

    for body_data in mjcf_data.get("bodies", []):
        link_name = body_data.get("name")
        if not link_name:
            continue
        lk = ensure_link(link_name)
        visuals = body_data.get("visuals") or []
        for visual_data in visuals:
            mesh_path = visual_data.get("mesh")
            if not mesh_path:
                continue
            vis = {
                "mesh": mesh_path,
                "origin_xyz": list(visual_data.get("pos", [0.0, 0.0, 0.0])),
                "origin_rpy": quat_to_rpy_xyzw(visual_data.get("quat", [1.0, 0.0, 0.0, 0.0])),
                "color": list(visual_data.get("color", body_data.get("color", [0.8, 0.8, 0.8, 1.0]))[:4]),
                "scale": list(visual_data.get("scale", [1.0, 1.0, 1.0])),
            }
            lk.visuals.append(vis)
            if lk.visual_mesh is None:
                lk.visual_mesh = mesh_path
                lk.visual_origin_xyz = vis["origin_xyz"]
                lk.visual_origin_rpy = vis["origin_rpy"]

        if not lk.visuals and body_data.get("stl_file"):
            vis = {
                "mesh": body_data.get("stl_file"),
                "origin_xyz": list(body_data.get("visual_origin", {}).get("xyz", [0.0, 0.0, 0.0])),
                "origin_rpy": list(body_data.get("visual_origin", {}).get("rpy", [0.0, 0.0, 0.0])),
                "color": list(body_data.get("color", [0.8, 0.8, 0.8, 1.0])[:4]),
                "scale": list(body_data.get("mesh_scale", [1.0, 1.0, 1.0])),
            }
            lk.visuals.append(vis)
            lk.visual_mesh = vis["mesh"]
            lk.visual_origin_xyz = vis["origin_xyz"]
            lk.visual_origin_rpy = vis["origin_rpy"]

    child_links = set()
    for joint_data in mjcf_data.get("joints", []):
        parent_link = joint_data.get("parent") or "base_link"
        child_link = joint_data.get("child")
        if not child_link:
            continue
        ensure_link(parent_link)
        ensure_link(child_link)

        jt = URDFJoint()
        jt.name = joint_data.get("name") or f"{parent_link}_to_{child_link}"
        joint_type = joint_data.get("type", "fixed")
        jt.joint_type = joint_type if joint_type in ("revolute", "continuous", "fixed") else "fixed"
        jt.parent_link = parent_link
        jt.child_link = child_link
        # Joints directly from synthetic base_link carry the MJCF worldbody placement
        # offset (simulation standing height). Strip xyz so the root body appears at
        # the 3D-view origin. Preserve rpy so body orientations are not lost.
        # Skip self-referential joints (base_link → base_link from freejoint bodies).
        if parent_link == child_link:
            continue
        if parent_link == "base_link":
            jt.origin_xyz = [0.0, 0.0, 0.0]
            jt.origin_rpy = list(joint_data.get("origin_rpy", [0.0, 0.0, 0.0]))
        else:
            jt.origin_xyz = list(joint_data.get("origin_xyz", [0.0, 0.0, 0.0]))
            jt.origin_rpy = list(joint_data.get("origin_rpy", [0.0, 0.0, 0.0]))
        jt.axis = list(joint_data.get("axis", [1.0, 0.0, 0.0]))

        limit = joint_data.get("limit") or {}
        jt.limit_lower = math.degrees(float(limit.get("lower", -math.pi)))
        jt.limit_upper = math.degrees(float(limit.get("upper", math.pi)))
        if jt.joint_type == "continuous":
            jt.limit_lower = -180.0
            jt.limit_upper = 180.0
            jt.joint_type = "revolute"

        rm.joints[jt.name] = jt
        rm.parent_map[child_link] = jt.name
        rm.child_map.setdefault(parent_link, []).append(jt.name)
        child_links.add(child_link)
        if jt.joint_type == "revolute":
            rm.joint_order.append(jt.name)

    all_link_names = set(rm.links.keys())
    roots = all_link_names - child_links
    if "base_link" in roots:
        rm.root_link = "base_link"
    elif roots:
        rm.root_link = list(roots)[0]
    elif rm.links:
        rm.root_link = list(rm.links.keys())[0]

    print(f"[MJCFRobotModel] Parsed {len(rm.links)} links, "
...
def build_robot_model_from_urdf(urdf_path, urdf_data):
    """Build the motion editor's robot model from URDFParser output.

    Args:
        urdf_path: Path to the URDF file (or xacro/SDF file)
        urdf_data: Dictionary returned by URDFParser.parse_urdf()

    Returns:
        URDFRobotModel instance populated with links and joints
    """
    rm = URDFRobotModel()
    rm.urdf_path = urdf_path
    rm.urdf_dir = os.path.dirname(os.path.abspath(urdf_path))
    rm.urdf_data = urdf_data
    rm.links.clear()
    rm.joints.clear()
    rm.joint_order.clear()
    rm.parent_map.clear()
    ...
    def resolve_mesh_path(mesh_path):
        """メッシュパスを解決するヘルパー関数"""
        if not mesh_path:
            return None
        ...
        # URDFRobotModelの_resolve_mesh_pathを使用
        resolved = rm._resolve_mesh_path(mesh_path)
        if resolved and os.path.exists(resolved):
            return resolved

        # ファイルベース名で検索
        mesh_basename = os.path.basename(mesh_path)
        found = rm._find_mesh_file(mesh_basename)
        if found:
            return found
    ...
    # Parse links from urdf_data
    links_data = urdf_data.get('links', {})
    mesh_found_count = 0
    mesh_not_found = []
    ...
    for link_name, link_info in links_data.items():
        ...
        # Get visual mesh (main visual)
        stl_file = link_info.get('stl_file')
        if stl_file:
            # メッシュパスを解決
            resolved_stl = resolve_mesh_path(stl_file)
            if resolved_stl and os.path.exists(resolved_stl):
                stl_file = resolved_stl
                mesh_found_count += 1
            else:
                mesh_not_found.append((link_name, stl_file))

            visual_origin = link_info.get('visual_origin', {'xyz': [0, 0, 0], 'rpy': [0, 0, 0]})
            color = link_info.get('color', [0.8, 0.8, 0.8, 1.0])
            mesh_scale = link_info.get('mesh_scale', [1.0, 1.0, 1.0])

            vis = {
                "mesh": stl_file,
                "origin_xyz": list(visual_origin.get('xyz', [0.0, 0.0, 0.0])),
                "origin_rpy": list(visual_origin.get('rpy', [0.0, 0.0, 0.0])),
                "color": list(color[:4]) if len(color) >= 4 else list(color) + [1.0],
                "scale": list(mesh_scale),
            }
            lk.visuals.append(vis)
            lk.visual_mesh = stl_file
            lk.visual_origin_xyz = vis["origin_xyz"]
            lk.visual_origin_rpy = vis["origin_rpy"]

        # Process decoration visuals
        decorations = link_info.get('decorations', [])
        for dec in decorations:
            dec_mesh = dec.get('stl_file')
            if dec_mesh:
                # メッシュパスを解決
                resolved_dec = resolve_mesh_path(dec_mesh)
                if resolved_dec and os.path.exists(resolved_dec):
                    dec_mesh = resolved_dec
                    mesh_found_count += 1
                else:
                    mesh_not_found.append((link_name, dec_mesh))

                dec_origin = dec.get('visual_origin', {'xyz': [0, 0, 0], 'rpy': [0, 0, 0]})
                dec_color = dec.get('color', [0.8, 0.8, 0.8, 1.0])
                dec_scale = dec.get('mesh_scale', [1.0, 1.0, 1.0])

                vis = {
                    "mesh": dec_mesh,
                    "origin_xyz": list(dec_origin.get('xyz', [0.0, 0.0, 0.0])),
                    "origin_rpy": list(dec_origin.get('rpy', [0.0, 0.0, 0.0])),
                    "color": list(dec_color[:4]) if len(dec_color) >= 4 else list(dec_color) + [1.0],
                    "scale": list(dec_scale),
                }
                lk.visuals.append(vis)

    # Parse joints from urdf_data
    joints_data = urdf_data.get('joints', [])
    child_links = set()

    for joint_info in joints_data:
        parent_link = joint_info.get('parent') or "base_link"
        child_link = joint_info.get('child')
        if not child_link:
            continue
        ...
        jt = URDFJoint()
        jt.name = joint_info.get('name') or f"{parent_link}_to_{child_link}"
        joint_type = joint_info.get('type', 'fixed')
        # Map joint types to supported types
        if joint_type in ('revolute', 'continuous', 'prismatic'):
            jt.joint_type = 'revolute' if joint_type in ('revolute', 'continuous') else 'fixed'
        else:
            jt.joint_type = 'fixed'
        jt.parent_link = parent_link
        jt.child_link = child_link
        jt.origin_xyz = list(joint_info.get('origin_xyz', [0.0, 0.0, 0.0]))
        jt.origin_rpy = list(joint_info.get('origin_rpy', [0.0, 0.0, 0.0]))
        jt.axis = list(joint_info.get('axis', [1.0, 0.0, 0.0]))

        # Parse limits (convert from radians to degrees)
    ...
    # ログ出力
    print(f"[URDFRobotModel] Built from parsed data: {len(rm.links)} links, "
          f"{len(rm.joints)} joints, "
          f"{len(rm.joint_order)} revolute joints, "
          f"root={rm.root_link}")
    print(f"[URDFRobotModel] Meshes found: {mesh_found_count}")
    if mesh_not_found:
        print(f"[URDFRobotModel] Meshes not found ({len(mesh_not_found)}):")
        for link_name, mesh_path in mesh_not_found[:10]:  # 最初の10件のみ表示
            print(f"  - {link_name}: {mesh_path}")
        if len(mesh_not_found) > 10:
            print(f"  ... and {len(mesh_not_found) - 10} more")
    return rm
...
def _show_launch_guard_message():
    """Show a modal when this module is run as a script."""
    app = QtWidgets.QApplication.instance()
    if app is None:
        app = QtWidgets.QApplication([])
    box = QtWidgets.QMessageBox()
    box.setWindowTitle("Legacy Motion Editor")
    box.setText("Please open Legacy Motion Editor First.")
...
if __name__ == "__main__":
    import sys
    sys.exit(_show_launch_guard_message())
